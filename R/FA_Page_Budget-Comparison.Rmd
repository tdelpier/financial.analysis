...
FA_Data_District %>% 
  ...
  set_header_labels(FY = "Fiscal Year",
                    ...
                    fid.r.total.audit.lag1 = "Prior Year's Actual Revenue",
                    est.1.rev.total = "MEA Revenue Estimate",
                    db.orig.rev.pct.act = "Original Budget % of Actual",
                    fid.r.total.audit.lag1.pct.act = "Prior Year's Revenue % of Current Actual",
                    est.1.rev.total.pct.act = "MEA Estimate % of Actual") %>%
...
district_budget_data %>% 
  ...
  mutate(name = case_when( name == "db.orig.rev.pct.act"            ~ "Original Budget",
                           name == "fid.r.total.audit.lag1.pct.act" ~ "  Prior Year        ",
  ...
  labs(title = 
"<b>District's <span style='color: #003057'> Original Budget</span></b> , <b><span style='color:#00A651 '>Prior Year's</span></b> , and
...
FA_Data_District %>% 
  filter(FY > (fiscal.year - prior.years.to.show),
         !is.na(est.1.rev.total.pct.act)) %>% 
  select(FY, fid.r.total.audit, db.original.total.revenue, 
         db.final.total.revenue, db.orig.rev.error, db.final.rev.error) %>% 
  flextable() %>% 
  set_header_labels(
                  FY = "Fiscal Year",
                  fid.r.total.audit = "Actual Revenue",
...
FA_Data_District %>% 
  filter(FY > (fiscal.year - prior.years.to.show),
         !is.na(est.1.rev.total.pct.act)) %>% 
  select(FY, fid.e.total.xtrans, db.original.total.expenditure, 
         db.final.total.expenditure, db.orig.exp.error, db.final.exp.error) %>% 
  flextable() %>% 
  set_header_labels(
                  FY = "Fiscal Year",
