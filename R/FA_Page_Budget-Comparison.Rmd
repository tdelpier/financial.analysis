...
prior.years.to.show <- 10

```

## Budget Comparison

Table xx and figure x are intended to help asses the accuracy of the **current MEA revenue estimate** compared to the district's original budget. Consequently, table xx and figure xx do not show the MEA revenue estimated in that year. Rather, they apply the current mechanical model used to estimate current revenue to historical data. Note, **baseline revenue** assumes no change in revenue from the previous fiscal year. 
...
Table xx and figure xx compares the surplus or deficit expected by the district in their original budget  to the actual surplus or deficit. A **surplus** occurs when revenue is greater than expenditure within a fiscal year. A **deficit** occurs when revenue is less than expenditure within a fiscal year. 
...
FA_Data_District %>%
  ...
  flextable_custom_theme(title = "Table x. Surplus/Deficit, Original Budget and Actual")
...
FA_Data_District %>%
  ...
  labs(title = "Figure x. Surplus/Deficit",
       subtitle = "<span style='color: #C2002F '> Original Budget</span> vs. <span style='color: #000000'>Actual</span>",
       x = "Fiscal Year",
       y = "") +
  scale_color_manual(name = "", values = c("Original Budget" = mea_red, "Actual" = "black"))+
...
Tables xx and xx compare General Fund revenue and expenditure expected in the district's original and final budgets to the actual revenue earned and expenditure spent in that fiscal year. 
...
FA_Data_District %>% 
  ...
  flextable_custom_theme(title = "Table x. Budgeted General Fund Revenue")
...
FA_Data_District %>% 
  ...
  labs(title = "Figure x. Original and Final Budgeted Revenue % of Actual",
       x = "Fiscal Year",
       y = "") +
...
FA_Data_District %>% 
  ...
  bg(j = "fid.r.total.audit", bg = mea_gray_light, part = "all") %>%
  flextable_custom_theme(title = "Table x. Budgeted General Fund Expenditure")
```
...
#   filter(FY > (fiscal.year - prior.years.to.show),
#          FY < fiscal.year) %>% 
#   select(FY, db.original.total.expenditure, db.final.total.expenditure, 
#          fid.e.total.xtrans, db.orig.exp.error, db.final.exp.error) %>% 
#   flextable() %>% 
#   set_header_labels(
#                   FY = "Fiscal Year",
#                   fid.e.total.xtrans = "Actual",
#                   db.original.total.expenditure = "Original Budget",
#                   db.final.total.expenditure = "Final Budget",
...
FA_Data_District %>% 
  ...
  scale_x_continuous(breaks = seq(1990, 2100, by = 1) )+
  scale_y_continuous(labels = number_format(suffix = "%")) +
  labs(title = "Figure x. Original and Final Budgeted Expenditure % of Actual",
       x = "Fiscal Year",
       y = "") +
  scale_color_manual(values = c(mea_blue, mea_red))+
  scale_linetype_manual(values=c(1, 2))+
