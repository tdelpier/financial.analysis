...
No_FID_Avalible <- 
  ...
  select(fid.r.total) %>% 
  mutate(no.fid.avalible = ifelse(is.na(fid.r.total) | fid.r.total == 0 , 1, 0)) %>%
  pull()
...
if(No_FID_Avalible == 0 ){
  ...
  FA_Data_District %>%
    ...
    flextable_custom_theme(title = "Table 1. Revenue Estimate")
...
FA_Data_District %>%
  ungroup() %>%
  filter(if(No_FID_Avalible == 1){FY <= fiscal.year - 2}else{FY>=2000}) %>%
  ...
  flextable_custom_theme(title = "Table 2. Revenue Less Stimulus, UAAL, and Other Net")


```

Note: "Total less Stimulus and Net Zero" is the preferred measure to fill out MEA's Cost Alignment Calculator (CAC). These dollars represent revenues that are not one-time federal stimulus nor net zero payments (i.e., those appropriations that come with a corresponding cost increase). Specifically, the green column excludes federal stimulus as well as sections 27l(2), 27l(4), 147a(4), 147c(1), 147c(2), and 147g. These stimulus and zero net revenues are highlighted in gray in tables 1 and 2. 
...
FA_Data_District %>%
  ...
  labs(title = "Figure 1. <span style='color: #003057'> Total Revenue</span> and <span style='color: #00A651'> Total less Stimulus and Net Zero</span>",
       x = "Fiscal Year",
       y = "")+
