...
FA_Data_District %>%
  ...
  flextable() %>% 
  add_header_lines(values = "General Fund Balance") %>% 
...
FA_Data_District %>% 
  ...
  flextable() %>% 
  add_header_lines(values = "General Fund Balance by Balance Class") %>% 
  set_header_labels(FY = "Fiscal Year",
...
While the General Fund typically contains most resources used on district operations, other funds provide financial transparency for food service, capital financing, and other auxiliary services. Note that the names for non-General funds may not match the audit. While district's have flexibility in fund naming internally, fund names are standardized for state reporting. Additionally, districts with multiple funds of the same kind are combined in this report. For more detail, refer to the audit. 
...
FA_Data_District %>% 
  ...
  flextable() %>% 
  add_header_lines(values = "Other Fund Balancesv by Fiscal Year") %>% 
  colformat_double(big.mark = ",",
                   digits = 0) %>% 
...
Warning: School district classification of non-General fund balances may be inaccurate. Occasionally, school districts will classify revenue for a bonded capital project or other restricted revenue source as assigned or committed. If this is the case, the transfer back calculation reported in the Transfers section of this report will be incorrect. 
